...
public class OrderController {
	...
	private Map<String, Object> getOrderListByShopId(HttpServletRequest request) {
		...
		try {
			...
			for(int i=0;i<se.getCount();i++)
			{
				...
				List<OrderInfo> orderList = ore.getOrderList();
				for (OrderInfo orderInfo : orderList){
					PersonInfo personInfo = personInfoService.getPersonInfoByUserId(orderInfo.getUserId());
					orderInfo.setUserName(personInfo.getUserName());
				}
				ore.setOrderList(orderList);
	...
	@ResponseBody
	@ApiOperation(value = "按条件搜索订单")
	private Map<String, Object> searchOrderByCondition(
			@RequestBody @ApiParam(name = "OrderCondition", value = "传入json格式", required = true)OrderCondition orderCondition, HttpServletRequest request) {
		Map<String, Object> modelMap = new HashMap<String, Object>();
		// 空值判断
		if (orderCondition != null) {
			try {
				...
				String content = orderCondition.getContent();
				if(orderCondition.getWayOfSearch()==0){
					orderCondition.setServiceName(content);
				}else if (orderCondition.getWayOfSearch()==1){
					orderCondition.setUserName(content);
				}else if (orderCondition.getWayOfSearch()==2){
					orderCondition.setPhone(content);
				}else if (orderCondition.getWayOfSearch()==3){
					orderCondition.setEmail(content);
				};
				System.out.println(orderCondition.toString());
				OrderExecution ore = OrderService.getOrderList3(orderCondition);
				ArrayList<ServiceInfo> serviceInfos = new ArrayList<>();
				for (OrderInfo orderInfo :
						ore.getOrderList()) {
					ServiceInfo byServiceId = sService.getByServiceId(orderInfo.getServiceId());
					serviceInfos.add(byServiceId);
				}
				if (ore.getOrderList()!=null) {
					modelMap.put("OrderList", ore.getOrderList());
					modelMap.put("serviceList",serviceInfos);
					modelMap.put("success", true);
				} else {
					modelMap.put("success", false);
